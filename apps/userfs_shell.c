...
static void show_help(void)
{
    ...
    printf("  format                       create a fresh 16 MiB image\n");
    ...
    printf("  lstat PATH                   show link metadata without following\n");
    printf("  ln OLD NEW                   create a hard link\n");
    printf("  ln -s TARGET LINK            create a symbolic link\n");
    printf("  readlink PATH                print a symbolic-link target\n");
    printf("  chmod MODE PATH              change permissions (for example 600)\n");
    printf("  truncate PATH SIZE           resize a file\n");
    printf("  mmap-show PATH OFFSET SIZE   display a copy-backed mapping\n");
    printf("  mmap-write PATH OFFSET TEXT  modify through shared mapping\n");
    ...
    printf("\nOther commands:\n");
    printf("  features                     summarize implemented features\n");
...
static void command_features(void)
{
    printf("\nUserFS Features\n");
    printf("------------------------------------------------------------\n");
    printf("Disk image          : 16 MiB (32768 x 512-byte blocks)\n");
    printf("Maximum inodes      : 512 persistent inodes\n");
    printf("Block addressing    : 8 direct + single + double indirect\n");
    printf("Maximum file size   : 8,458,240 bytes\n");
    printf("Directories         : persistent nested directories\n");
    printf("Hard links          : supported with persistent link counts\n");
    printf("Symbolic links      : absolute/relative, ELOOP protection\n");
    printf("Metadata            : permissions, UID/GID and timestamps\n");
    printf("Journal             : after-image redo with mount recovery\n");
    printf("Memory mapping      : copy-backed shared/private mappings\n");
    printf("Thread safety       : serialized public operations\n");
    printf("Process safety      : whole-image advisory fcntl locking\n");
    printf("Persistence         : successful changes survive remount\n");
    printf("Verification        : strict tests plus ASan and UBSan\n");
    printf("------------------------------------------------------------\n");
    printf("Note: mmap is copy-backed, not native kernel mmap.\n");
    printf("Note: the journal does not make every operation crash-atomic.\n\n");
}
...
static void command_format(void)
{
    ...
    printf("OK: formatted %s (16 MiB, 32768 blocks).\n", image_path);
}
...
static void command_ls(const char *path)
{
    ...
    for (index = 0; index < count; ++index) {
        const char *type_name = entries[index].type == UFS_TYPE_DIR ? "directory" :
                                entries[index].type == UFS_TYPE_SYMLINK ? "symlink" :
                                "file";
        printf("%-32s %-9s %zu\n", entries[index].name, type_name,
               entries[index].size);
    }
    printf("%d item(s)\n", count);
}

static void command_stat(const char *path, int nofollow)
{
    char resolved[UFS_MAX_PATH + 1];
    struct ufs_stat status;
    ...
    if ((nofollow ? ufs_lstat(resolved, &status) :
                    ufs_stat(resolved, &status)) != 0) {
        print_error(nofollow ? "lstat" : "stat");
        return;
    }
    printf("type:        %s\n",
           status.type == UFS_TYPE_DIR ? "directory" :
           status.type == UFS_TYPE_SYMLINK ? "symbolic link" : "file");
    printf("size:        %llu bytes\n", (unsigned long long)status.size);
    printf("blocks:      %u\n", (unsigned int)status.blocks);
    printf("mode:        %03o\n", (unsigned int)(status.mode & 0777U));
    printf("owner uid:   %u\n", (unsigned int)status.uid);
    printf("owner gid:   %u\n", (unsigned int)status.gid);
    printf("link count:  %u\n", (unsigned int)status.link_count);
    printf("atime:       %lld.%09ld\n", (long long)status.atime.tv_sec,
           status.atime.tv_nsec);
    printf("mtime:       %lld.%09ld\n", (long long)status.mtime.tv_sec,
           status.mtime.tv_nsec);
    printf("ctime:       %lld.%09ld\n", (long long)status.ctime.tv_sec,
           status.ctime.tv_nsec);
}

static void command_link(const char *first, const char *second,
                         const char *third)
{
    char source[UFS_MAX_PATH + 1];
    char destination[UFS_MAX_PATH + 1];
    int symbolic = first != NULL && strcmp(first, "-s") == 0;
    const char *old_or_target = symbolic ? second : first;
    const char *new_path = symbolic ? third : second;

    if (!require_shell_mount() || old_or_target == NULL || new_path == NULL) {
        printf("Usage: ln OLD NEW  or  ln -s TARGET LINK\n");
        return;
    }
    if (resolve_shell_path(new_path, 0, destination) != 0) {
        return;
    }
    if (symbolic) {
        if (ufs_symlink(old_or_target, destination) != 0) {
            print_error("ln -s");
            return;
        }
    } else {
        if (resolve_shell_path(old_or_target, 0, source) != 0) {
            return;
        }
        if (ufs_link(source, destination) != 0) {
            print_error("ln");
            return;
        }
    }
    printf("OK: created %s link %s.\n", symbolic ? "symbolic" : "hard",
           destination);
}

static void command_readlink(const char *path)
{
    char resolved[UFS_MAX_PATH + 1];
    ...
    length = ufs_readlink(resolved, target, UFS_MAX_PATH);
    if (length < 0) {
...
static int parse_mapping_number(const char *text, uint64_t *value)
{
    char *end;
...
static void command_mmap_show(const char *path, const char *offset_text,
                              const char *length_text)
{
    ...
    if (!require_shell_mount() || path == NULL || offset_text == NULL ||
        length_text == NULL) {
        printf("Usage: mmap-show PATH OFFSET SIZE\n");
        return;
    }
    if (parse_mapping_number(offset_text, &offset) < 0 ||
        parse_mapping_number(length_text, &length) < 0 || length == 0 ||
        offset > (uint64_t)INT64_MAX || length > (uint64_t)SIZE_MAX) {
        printf("ERROR: mmap-show: invalid offset or size\n");
        return;
    }
    if (resolve_shell_path(path, 0, resolved) != 0) {
        return;
    ...
    if (fd < 0) {
        print_error("mmap-show open");
        return;
    }
    mapping = ufs_mmap(fd, (size_t)length, UFS_PROT_READ,
                       UFS_MAP_PRIVATE, (off_t)offset);
    if (mapping == UFS_MAP_FAILED) {
        print_error("mmap-show");
        (void)ufs_close(fd);
        return;
    }
    printf("Mapped bytes: ");
    (void)fwrite(mapping, 1, (size_t)length, stdout);
    printf("\n");
    if (ufs_munmap(mapping, (size_t)length) < 0) {
        print_error("mmap-show munmap");
    }
    (void)ufs_close(fd);
}

static void command_mmap_write(const char *path, const char *offset_text,
                               const char *text)
{
    char resolved[UFS_MAX_PATH + 1];
    uint64_t offset;
    ...
    if (!require_shell_mount() || path == NULL || offset_text == NULL ||
        text == NULL || *text == '\0') {
        printf("Usage: mmap-write PATH OFFSET TEXT\n");
        return;
    }
    if (parse_mapping_number(offset_text, &offset) < 0 ||
        offset > (uint64_t)INT64_MAX) {
        printf("ERROR: mmap-write: invalid offset\n");
        return;
    }
    if (resolve_shell_path(path, 0, resolved) != 0) {
        return;
    ...
    length = strlen(text);
    fd = ufs_open(resolved, UFS_O_RDWR);
    if (fd < 0) {
        print_error("mmap-write open");
        return;
    }
    mapping = ufs_mmap(fd, length, UFS_PROT_READ | UFS_PROT_WRITE,
                       UFS_MAP_SHARED, (off_t)offset);
    if (mapping == UFS_MAP_FAILED) {
        print_error("mmap-write");
        (void)ufs_close(fd);
        return;
    }
    memcpy(mapping, text, length);
    if (ufs_msync(mapping, length) < 0 ||
        ufs_munmap(mapping, length) < 0) {
        print_error("mmap-write sync/unmap");
        (void)ufs_close(fd);
        return;
    }
    (void)ufs_close(fd);
    printf("OK: mapped %zu byte(s) at offset %llu and synchronized them.\n",
           length, (unsigned long long)offset);
}

static void command_chmod(const char *mode_text, const char *path)
{
    char resolved[UFS_MAX_PATH + 1];
    char *end;
    unsigned long mode;

    if (!require_shell_mount() || mode_text == NULL || path == NULL) {
        printf("Usage: chmod MODE PATH\n");
        return;
    }
    errno = 0;
    mode = strtoul(mode_text, &end, 8);
    if (errno != 0 || *mode_text == '\0' || *end != '\0' || mode > 07777U) {
        printf("ERROR: chmod: invalid octal mode\n");
        return;
    }
    if (resolve_shell_path(path, 0, resolved) != 0) {
        return;
    }
    if (ufs_chmod(resolved, (mode_t)mode) < 0) {
        print_error("chmod");
        return;
    }
    printf("OK: mode of %s changed to %03lo.\n", resolved, mode);
}
...
static void command_super(void)
{
    ...
    printf("journal blocks:     %u-%u\n",
           (unsigned int)g_ufs.sb.journal_start,
           (unsigned int)(g_ufs.sb.journal_start +
                          g_ufs.sb.journal_blocks - 1U));
    printf("filesystem state:   %s\n",
           g_ufs.sb.state == UFS_STATE_CLEAN ? "clean" : "mounted/dirty");
...
static int execute_line(char *line)
{
    ...
    if (strcmp(command, "features") == 0) {
        ...
        command_ls(strtok(NULL, " \t\r\n"));
               ...
               strcmp(command, "lstat") == 0) {
        command_stat(strtok(NULL, " \t\r\n"), strcmp(command, "lstat") == 0);
    } else if (strcmp(command, "ln") == 0) {
        char *first = strtok(NULL, " \t\r\n");
        char *second = strtok(NULL, " \t\r\n");
        command_link(first, second, strtok(NULL, " \t\r\n"));
    } else if (strcmp(command, "readlink") == 0) {
        command_readlink(strtok(NULL, " \t\r\n"));
    } else if (strcmp(command, "chmod") == 0) {
        char *mode_text = strtok(NULL, " \t\r\n");
        command_chmod(mode_text, strtok(NULL, " \t\r\n"));
    } else if (strcmp(command, "truncate") == 0) {
        path = strtok(NULL, " \t\r\n");
        command_truncate(path, strtok(NULL, " \t\r\n"));
    } else if (strcmp(command, "mmap-show") == 0) {
        path = strtok(NULL, " \t\r\n");
        {
            char *offset_text = strtok(NULL, " \t\r\n");
            command_mmap_show(path, offset_text, strtok(NULL, " \t\r\n"));
        }
    } else if (strcmp(command, "mmap-write") == 0) {
        char *offset_text;
        char *text;
        path = strtok(NULL, " \t\r\n");
        offset_text = strtok(NULL, " \t\r\n");
        text = skip_spaces(strtok(NULL, "\r\n"));
        command_mmap_write(path, offset_text, text);
