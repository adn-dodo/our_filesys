...
pos = ufs_seek(fd, -1, SEEK_SET);

check(pos == -1, "negative seek fails");
check(errno == EINVAL, "negative seek gives EINVAL");
    ufs_close(fd);
...
static void test_append(void)
{
    ...
    fd = ufs_open("/file.txt", UFS_O_RDONLY);

    check(fd >= 0, "open for reading succeeds");

    if (fd >= 0) {
        char buffer[20] = {0};

        n = ufs_read(fd, buffer, 11);

        check(n == 11,
              "read appended data succeeds");

        check(strcmp(buffer, "Hello World") == 0,
              "append produces correct contents");
...
int main(void)
{
    ...
    test_nonempty_directory();
    test_unlink_directory();
    test_rmdir_file();
