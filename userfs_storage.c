 ...
 *
 * This is a draft: it depends on a handful of helpers that are owned by
 * other members and are only declared (not yet defined) in
 * userfs_internal.h:
 *
 *   - g_ufs, ufs_read_block(), ufs_write_block(), ufs_flush_superblock()
 *       -> Member 1 (lifecycle / image I/O)
 *   - resolve_path()
 *       -> Member 3 (paths / namespace)
 *
 * Until those land, this file will compile but not link on its own; see
 * the bottom of this comment block in DESIGN.md notes for a temporary
...
int allocate_inode(uint32_t *inode_num_out)
{
    ...
    for (uint32_t i = 0; i < UFS_MAX_INODES; i++) {
        ...
        inode.size = 0;
        inode.block_count = 0;
        for (uint32_t d = 0; d < UFS_DIRECT_BLOCKS; d++) {
            inode.direct[d] = UFS_INVALID_BLOCK;
        }
...
int free_inode(uint32_t inode_num)
{
    ...
    /* Reset the on-disk inode so stale pointers can never be reused. */
    struct ufs_inode inode;
    memset(&inode, 0, sizeof(inode));
    for (uint32_t d = 0; d < UFS_DIRECT_BLOCKS; d++) {
        inode.direct[d] = UFS_INVALID_BLOCK;
    ...
    inode.single_indirect = UFS_INVALID_BLOCK;
    inode.double_indirect = UFS_INVALID_BLOCK;
    (void)write_inode(inode_num, &inode);
