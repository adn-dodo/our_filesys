...
static int ufs_truncate_unlocked(const char *path, size_t size)
{
    struct ufs_inode inode;
    ...
    if (ufs_check_access_inode(&inode, W_OK) < 0) {
        return -1;
    }
    if (ufs_truncate_inode(inode_num, &inode, size) < 0) {
        return -1;
    }

    ufs_touch_mtime_ctime(&inode);
    if (ufs_tx_begin(&tx) < 0 ||
        ufs_tx_stage_inode(&tx, inode_num, &inode) < 0 ||
        ufs_tx_commit(&tx) < 0) {
        ufs_tx_abort(&tx);
        return -1;
    }
...
int ufs_truncate(const char *path, size_t size)
{
    ...
    result = ufs_truncate_unlocked(path, size);
    saved_errno = errno;
