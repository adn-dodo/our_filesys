...
static int persist_inode_metadata(uint32_t inode_number,
...
int file_io_get_fd_info(int fd, struct file_io_fd_info *info)
{
    ...
    info->inode_number = descriptor_table[fd].inode_number;
    info->inode_generation = descriptor_table[fd].inode_generation;
    info->flags = descriptor_table[fd].flags;
    return 0;
}
...
static int ufs_open_unlocked(const char *path, int flags)
{
    uint32_t inode_number;
    ...
    result = resolve_path_ex(path, UFS_RESOLVE_FOLLOW_FINAL, &inode_number);
    if (result < 0) {
    ...
    requested_access = 0;
    if ((flags & UFS_O_RDWR) != UFS_O_WRONLY) {
        requested_access |= R_OK;
    }
    if ((flags & UFS_O_RDWR) != UFS_O_RDONLY) {
        requested_access |= W_OK;
    }
    if (ufs_check_access_inode(&inode, requested_access) < 0) {
        return -1;
    }
    ...
    descriptor_table[fd].inode_number = inode_number;
    descriptor_table[fd].inode_generation = inode.generation;
...
static int ufs_close_unlocked(int fd)
{
...
static ssize_t ufs_read_unlocked(int fd, void *buf, size_t count)
{
    struct ufs_inode inode;
    ...
    if (inode.generation != descriptor_table[fd].inode_generation) {
        errno = EBADF;
        return -1;
    }
    ...
    TRACE("[I/O] read fd=%d offset=%lld count=%zu\n", fd,
          (long long)descriptor_table[fd].offset, count);
    while (total < count) {
        ...
        if (result < 0 || physical_block == UFS_INVALID_BLOCK) {
            if (result >= 0) {
                errno = EIO;
            }
            if (total > 0) {
        ...
        result = ufs_read_block(physical_block, block);
        if (result < 0) {
            if (total > 0) {
                return (ssize_t)total;
            }
    ...
    TRACE("[I/O] read done fd=%d bytes=%zu offset=%lld\n", fd, total,
          (long long)descriptor_table[fd].offset);

    if (total > 0 && ufs_should_update_atime(&inode) == 1) {
        int saved_errno = errno;
        ufs_touch_atime(&inode);
        (void)persist_inode_metadata(descriptor_table[fd].inode_number,
                                     &inode);
        errno = saved_errno;
    }
    return (ssize_t)total;
...
static ssize_t ufs_write_unlocked(int fd, const void *buf, size_t count)
{
    struct ufs_inode inode;
    uint8_t block[UFS_BLOCK_SIZE];
    uint64_t start;
    ...
    while (total < count) {
        ...
        result = ufs_write_block(physical_block, block);
        if (result < 0) {
            if (total > 0) {
                descriptor_table[fd].offset = (off_t)(start + total);
                return (ssize_t)total;
    ...
    if (persist_inode_metadata(descriptor_table[fd].inode_number, &inode) < 0) {
        return -1;
    }
    TRACE("[I/O] write done fd=%d bytes=%zu offset=%lld\n", fd, total,
          (long long)descriptor_table[fd].offset);
    return (ssize_t)total;
}
...
static off_t ufs_seek_unlocked(int fd, off_t offset, int whence)
{
    struct ufs_inode inode;
    uint64_t base;
    off_t new_offset;
    ...
    if (read_inode(descriptor_table[fd].inode_number, &inode) < 0) {
        return (off_t)-1;
    }
    if (inode.generation != descriptor_table[fd].inode_generation) {
        errno = EBADF;
    ...
    TRACE("[I/O] seek fd=%d whence=%d change=%lld offset=%lld\n", fd,
          whence, (long long)offset, (long long)new_offset);
    return new_offset;
}
...
int ufs_open(const char *path, int flags)
{
    ...
    result = ufs_open_unlocked(path, flags);
    saved_errno = errno;
...
ssize_t ufs_read(int fd, void *buf, size_t count)
{
    ...
    result = ufs_read_unlocked(fd, buf, count);
    saved_errno = errno;
    ufs_operation_unlock();
    errno = saved_errno;
    return result;
...
ssize_t ufs_write(int fd, const void *buf, size_t count)
{
    ...
    result = ufs_write_unlocked(fd, buf, count);
    saved_errno = errno;
    ufs_operation_unlock();
    errno = saved_errno;
    return result;
...
off_t ufs_seek(int fd, off_t offset, int whence)
{
    ...
    result = ufs_seek_unlocked(fd, offset, whence);
    saved_errno = errno;
    ufs_operation_unlock();
    errno = saved_errno;
    return result;
